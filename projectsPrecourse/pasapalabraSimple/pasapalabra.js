...
const showRules = () => {
  ...
  alert(`Hola ${
    gameInfo.userName
  }, a continuación se te mostrará una definición de una palabra que deberás adivinar.
  \n-Cada acierto sumará un punto.\n-El tiempo maximo de la partida són ${
    gameInfo.maxTime / 1000
  } segundos.\n
...
const confirmPlay = () => {
  gameInfo.userWannaPlay = confirm(
    `Quieres empezar a jugar ${gameInfo.userName}?`
  );
...
const startGame = () => {
  ...
  while (
    ...
    gameInfo.wordsCorrect !== questions.length
  ) {
    initRound();
  }
};
...
const answerManager = () => {
  switch (gameInfo.userAnswer) {
    ...
    case questions[letter].answer:
      ...
      alert(
        `Correcto! ${gameInfo.wordsCorrect}/${gameInfo.wordsTotal} aciertos`
      );
      break;
    default:
      ...
      gameInfo.wordsAnswered += 1;
      alert(
        `Incorrecto, la respuesta correcta es: ${questions[letter].answer}`
      );
  }
};
...
const showRanking = (gameInfo) => {
  ...
  for (let i = 0; i < gameInfo.ranking.length; i++) {
    console.log(
      `${i + 1} position. ${gameInfo.ranking[i].player}: ${
        gameInfo.ranking[i].score
      } points`
    );
  }
};
